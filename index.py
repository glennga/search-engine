...
class StorageHandler:
    ...
    a) Each component is *immutable*. This allows us to simplify reasoning about flushing and merging, but at the cost
       of not being able to update values. ElasticSearch similarly adopts this practice of having immutable components.
    b) The merging process happens "all-at-once" once "close" is called (rather than a "rolling merge" approach).
    c) The resulting inverted index consists of a *single* file (as opposed to multiple).
    ...
    def close(self) -> str:
        ...
        :return The name of the index file generated.
        """
        ...
        while len(self.merge_queue) > 1:
            ...
            if len(self.memory_component) > 0:
                with open(self.merge_queue.popleft(), 'rb') as right_fp, \
                    ...
                    self._merge(left_component, right_component, out_fp)
    ...
    @staticmethod
    def _merge(left_component, right_component, out_fp):
        ...
        while True:
            if not is_left_exhausted and not is_right_exhausted:
                if left_token_pair[0] == right_token_pair[0]:
                    out_entry = (left_token_pair[0], heapq.merge(left_token_pair[1], right_token_pair[1]))
                    left_token_pair, is_left_exhausted = _advance(left_component)
                    right_token_pair, is_right_exhausted = _advance(right_component)
